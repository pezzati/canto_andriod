...
SuperpoweredNBandEQ::SuperpoweredNBandEQ(unsigned int samplerate, float *frequencies) {
    ...
    for (int n = 0; n < internals->numFilters; n++) {
        decibels[n] = 0.0f;
        float widthOctave = (frequencies[n + 1] > frequencies[n]) ? logf(
                frequencies[n + 1] / frequencies[n]) : logf(20000.0f / frequencies[n]);
...
void SuperpoweredNBandEQ::setBand(unsigned int index, float gainDecibels) {
    if (index < internals->numFilters) {
        ...
        internals->filters[index]->setParametricParameters(internals->filters[index]->frequency,
                                                           internals->filters[index]->octave,
                                                           gainDecibels);
    }
}

bool SuperpoweredNBandEQ::process(float *input, float *output, unsigned int numberOfSamples) {
    if (!input || !output || !numberOfSamples || !internals->numFilters)
        return false; // Some safety.
    ...
    if (newSamplerate > 0) {
        for (int n = 0; n < internals->numFilters; n++)
            internals->filters[n]->setSamplerate(newSamplerate);
    }
    ...
    if (internals->lastEnabled != enabled) {
        ...
        for (int n = 0; n < internals->numFilters; n++)
            internals->filters[n]->enable(internals->lastEnabled);
    }
